...
# This is passed as a parameter to the "run_clamav_scan.sh" script, i.e.
# run_clamav_scan.sh "/mnt"
#
...
# 	https://www.ixsystems.com/community/threads/how-to-install-clamav-on-freenas-v11.56790/#post-575177
#
...
# will not be sent, but if you hash (#) it out they are sent! For now have just hashed
...
### Usage ###
# To use this script you need to follow the instructions as below:
#
# 1) Creating Jail
...
# Hash out (#) the relevant line depending on whether you are using warden or iocage
...
### System/Script defined Variables (no need to edit/change these) ###
...
# Location and name of the log/temp files used by the script, they are automatically
# deleted at the end of the script.  Adding the Process ID makes them unique for
# concurrent runs.
...
run_tidyup()
{
# Delete all of the log and temp files, in preparation for a new set of files on
# the next run as well as to save space!
...
if [[ $jail_type = "iocage" ]] ; then
	iocage exec "${clamAVJailName}" clamscan -i -r -l ${clamscan_log} "${scan_location}"
else
